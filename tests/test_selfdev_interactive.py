...
def test_run_selfdev_uses_legacy_loop_when_flag_enabled(monkeypatch) -> None:
    args = SimpleNamespace(model="anthropic/test", max_turns=12, single=True, legacy=True)
    ...
    with patch("avoid_agent.__main__.load_dotenv"), \
         ...
         patch("avoid_agent.__main__._run_selfdev_interactive") as interactive, \
...
def test_run_selfdev_uses_interactive_by_default() -> None:
    args = SimpleNamespace(model="openai/gpt-5", max_turns=33, single=False, legacy=False)

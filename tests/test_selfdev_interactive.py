...
from avoid_agent.cli.tui import run as _run_agent
from avoid_agent.cli.selfdev import run as _run_selfdev
...
def test_run_selfdev_uses_legacy_loop_when_flag_enabled(monkeypatch) -> None:
    ...
    with patch("avoid_agent.cli.selfdev.load_dotenv"), \
         patch("avoid_agent.selfdev.loop.run_loop", side_effect=fake_run_loop), \
         patch("avoid_agent.cli.selfdev._run_selfdev_observe") as observe:
        exit_code = _run_selfdev(args)
...
def test_run_selfdev_uses_observe_by_default() -> None:
    ...
    with patch("avoid_agent.cli.selfdev.load_dotenv"), \
         ...
         patch("avoid_agent.cli.selfdev._run_selfdev_observe", return_value=42) as observe:
        exit_code = _run_selfdev(args)
...
def test_run_selfdev_uses_interactive_when_flag_set() -> None:
    ...
    with patch("avoid_agent.cli.selfdev.load_dotenv"), \
         patch("avoid_agent.selfdev.loop.run_loop") as run_loop, \
         patch("avoid_agent.cli.selfdev._run_selfdev_interactive", return_value=42) as interactive:
        exit_code = _run_selfdev(args)
...
def test_run_selfdev_uses_operator_when_flag_set() -> None:
    ...
    with patch("avoid_agent.cli.selfdev.load_dotenv"), \
         patch("avoid_agent.cli.selfdev._run_selfdev_operator", return_value=42) as operator:
        exit_code = _run_selfdev(args)
...
def test_self_improve_no_items_reports_info(monkeypatch) -> None:
    ...
    with patch("avoid_agent.cli.tui.TUI", FakeTUI), \
         patch("avoid_agent.selfdev.loop.parse_backlog", return_value=[]), \
         patch("avoid_agent.cli.tui.find_available_tools", return_value=[]), \
         patch("avoid_agent.cli.tui.providers.get_provider", return_value=MagicMock()), \
         patch("avoid_agent.cli.tui.build_system_prompt", return_value="sys"), \
         patch("avoid_agent.cli.shared.gather_initial_context", return_value=(".", "", "")), \
         patch("avoid_agent.cli.tui.get_saved_model", return_value="anthropic/test"), \
         patch("avoid_agent.cli.tui.load_user_config", return_value={}), \
         patch("avoid_agent.cli.tui.load_allowed", return_value=[]), \
         patch("avoid_agent.cli.tui.load_session", return_value=None), \
         patch("avoid_agent.cli.tui.load_dotenv"):
        _run_agent()
